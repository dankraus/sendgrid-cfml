    ...
    function run( testResults, testBox ){
        ...
        describe( "FileHandler", function(){
            it( "should handle files from the filesystem", function(){
                var handler = new SendGridCFML.models.FileHandler(path=files[2], filename='sg.txt');
            ...
            it( "should handle files from a url", function(){
                var handler = new SendGridCFML.models.FileHandler(url=uri, filename="esb.jpg");
            ...
            it( "should figure out the filename if one is not provided for path type", function(){
                var handler = new SendGridCFML.models.FileHandler(path=files[2]);
                expect(handler.type).toBe('path');
                expect(handler.path).toBe(files[2]);
            ...
            it( "should figure out the filename if one is not provided for url type", function(){
                var handler = new SendGridCFML.models.FileHandler(url=uri);
                expect(handler.type).toBe('url');
                expect(handler.url).toBe(uri);
            ...
            it( "should handle inline content", function(){
                ...
                var handler = new SendGridCFML.models.FileHandler(cid=cidData);
                expect(handler.type).toBe('cid');
                expect(handler.contentType).toBe('image/jpeg');
                expect(handler.cid).toBe('testcid');
            });
            it( "should throw error when filehandler cid is passed in and content, or contentType is has not been provided", function(){
                expect(function(){
            ...
            it( "should throw error when path, url, or, cid has not been provided", function(){
                expect(function(){
                    var handler = new SendGridCFML.models.FileHandler();
                }).toThrow("exception", "Unrecognized FileHandler type. Must pass in 'url', 'path', or 'cid'.");
            });
